...
public class Skip {
	public static void skip(User user, Guild guild, Message message) {
		if (user.isBot() || !message.getContentDisplay().toLowerCase().startsWith(Main.guildPrefix.get(guild.getId()) + "skip"))
			return;
		guild.loadMembers();
		...
		SendMessage.sendChannelMessage(message, "Skipped current track", "", false);
