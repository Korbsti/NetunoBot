...
public class Balance {
	public static void checkBalance(User user, Guild guild, Message message) {
		if(user.isBot() || (!message.getContentDisplay().toLowerCase().startsWith(Main.guildPrefix.get(guild.getId()) + "balance") && !message.getContentDisplay().toLowerCase().startsWith(Main.guildPrefix.get(guild.getId()) + "bal"))) return;
		...
		SendMessage.sendChannelMessage(message, "You have " + Main.userBalance.get(user.getId()) + "⌬ Nova Sapphires", "", false);
	}
}
