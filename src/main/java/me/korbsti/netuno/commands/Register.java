...
public class Register {
	...
	public static void registerUser(User user, Guild guild, Message message) {
		if (user.isBot() || !message.getContentDisplay().toLowerCase().startsWith(Main.guildPrefix.get(guild.getId()) + "register"))
			return;
		if (Main.isUserRegistered.get(user.getId()) == null) {
			...
			SendMessage.sendChannelMessage(message, "You are already registered in the database", "", false);
		}
